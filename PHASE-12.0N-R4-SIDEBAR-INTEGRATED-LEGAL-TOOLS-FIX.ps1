# ============================================================
# LITIGATION 360 LEOS
# PHASE 12.0N-R4 SIDEBAR-INTEGRATED LEGAL TOOLS FIX
#
# PURPOSE:
#   Remove the floating/overlapping Legal 360 dock and integrate
#   all new legal tools into the EXISTING left sidebar as normal
#   sidebar buttons.
#
# WHAT THIS FIXES:
#   - No more floating panel blocking existing 4 main buttons
#   - No more overlap with End User Workspace / Operations Centre /
#     Admin Centre / Developer Centre
#   - Legal tools remain on the left side only
#   - New tools become part of the same sidebar button structure
#
# WHAT THIS ADDS INSIDE EXISTING SIDEBAR:
#   - Legal Web Links
#   - Launch Apps / Docs
#   - Search Repository
#   - Instructions
...
#
# SAFE MODE:
#   - Backs up current enhancer JS/CSS/index.html
...
$IndexBackup = Backup-If-Exists -Source $IndexHtml -BackupFolder $RollbackRoot -Name "index.html.BACKUP-BEFORE-12.0N-R4"
$JsBackup = Backup-If-Exists -Source $JsPath -BackupFolder $RollbackRoot -Name "legal-management-enhancer.js.BACKUP-BEFORE-12.0N-R4"
$CssBackup = Backup-If-Exists -Source $CssPath -BackupFolder $RollbackRoot -Name "legal-management-enhancer.css.BACKUP-BEFORE-12.0N-R4"

Write-Pass "Backups created under:"
Write-Host $RollbackRoot -ForegroundColor Green

# ------------------------------------------------------------
# 4. CSS - no fixed left dock
# ------------------------------------------------------------
Write-Step "Writing sidebar-integrated CSS..."

$Css = @'
/* ============================================================
   LEOS sidebar-integrated legal tools
   No fixed floating dock. No overlap with original sidebar.
   ============================================================ */
...
.leos-sidebar-tool-btn:hover,
.leos-sidebar-tool-btn:focus {
  outline: none;
  border-color: rgba(201,166,70,0.86);
  background: #283447;
}

.leos-sidebar-tool-btn.leos-primary-tool {
  background: #fff8dd;
  border-color: #c9a646;
  color: #111827;
}
...
.leos-pro-drawer {
  ...
  left: 245px;
  top: 34px;
  width: min(880px, calc(100vw - 275px));
...
# ------------------------------------------------------------
# 5. JS - inject into existing sidebar only
# ------------------------------------------------------------
Write-Step "Writing sidebar-integrated legal enhancer JS..."
...
(function installSidebarIntegratedLegalTools() {
  ...
  function createToolButton(label, action, primary) {
  ...
  function injectSidebarTools() {
    ...
    title.className = "leos-sidebar-tools-title";
    title.textContent = "Legal Tools";
    toolBox.appendChild(title);

    toolBox.appendChild(createToolButton("Legal Web Links", "legal-links", true));
    toolBox.appendChild(createToolButton("Launch Apps / Docs", "launch", false));
    toolBox.appendChild(createToolButton("Search Repository", "search", false));
    toolBox.appendChild(createToolButton("Instructions", "instructions", false));
    toolBox.appendChild(createToolButton("Glossary", "glossary", false));
    toolBox.appendChild(createToolButton("Firm Info", "firm", false));
    toolBox.appendChild(createToolButton("Managing Partner", "partner", false));
    toolBox.appendChild(createToolButton("Settings", "settings", false));
...
It restores the previous legal enhancer behavior.
...
$Checklist = @"
# PHASE 12.0N-R4 POST-FIX CHECKLIST
...
The separate floating Legal 360 panel should be gone.

The existing left sidebar should still show:
...
Below those, inside the same sidebar, you should see:
...
Original 4 buttons visible:
YES / NO

No floating overlay blocking them:
YES / NO

Legal tools are inside existing sidebar:
YES / NO

Legal Web Links opens:
YES / NO

Launch Apps / Docs opens:
YES / NO

Search Repository opens:
YES / NO
...
$Report = @"
# PHASE 12.0N-R4 SIDEBAR-INTEGRATED LEGAL TOOLS FIX REPORT
...
- Removed floating legal tools dock behavior
- Integrated legal tools into existing sidebar
- Preserved original 4 main buttons
- Kept tools on left side only
- Kept Legal Web Links separate from Launch Apps / Docs
- Kept manual legal-only link manager
- Kept app/document launcher
- Kept Search, Instructions, Glossary, Firm Info, Managing Partner, Settings
...
New section below existing buttons:
...
Phase 12.0N-R4:
SIDEBAR-INTEGRATED LEGAL TOOLS FIX COMPLETE
...
$ReportPath = Join-Path $ReportRoot "PHASE-12.0N-R4-SIDEBAR-INTEGRATED-LEGAL-TOOLS-FIX-REPORT.md"
Save-Text -Path $ReportPath -Content $Report

Write-Host ""
Write-Pass "PHASE 12.0N-R4 SIDEBAR-INTEGRATED LEGAL TOOLS FIX COMPLETE"
Write-Host ""
Write-Host "Open report:" -ForegroundColor Cyan
Write-Host 'notepad "_LEOS_CONTROL\reports\PHASE-12.0N-R4-SIDEBAR-INTEGRATED-LEGAL-TOOLS-FIX-REPORT.md"'
Write-Host ""
