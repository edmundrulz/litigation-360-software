...
$Ops=Join-Path $Root "_operations"

$Phase=Join-Path $Ops "phase-10Y0-enterprise-master-registry-digital-twin"
$Reports=Join-Path $Phase "reports"
$Registries=Join-Path $Phase "registries"
$Graphs=Join-Path $Phase "graphs"
$Twins=Join-Path $Phase "twins"
$Docs=Join-Path $Phase "docs"
$Logs=Join-Path $Phase "logs"
$Validation=Join-Path $Phase "validation"
$Backups=Join-Path $Phase "backups"
$Debt=Join-Path $Phase "technical-debt"
$Impact=Join-Path $Phase "impact-analysis"

New-Item -ItemType Directory -Force -Path $Reports,$Registries,$Graphs,$Twins,$Docs,$Logs,$Validation,$Backups,$Debt,$Impact | Out-Null

$Script=Join-Path $Root "PHASE10Y0-ENTERPRISE-MASTER-REGISTRY-DIGITAL-TWIN.js"
...
Clear-Host
Write-Host "============================================================"
Write-Host "L360 PHASE 10Y.0 ENTERPRISE MASTER REGISTRY + DIGITAL TWIN"
Write-Host "============================================================"
...
main();
'@ | Out-File -LiteralPath $Script -Encoding UTF8

@"
# PHASE 10Y.0 ENTERPRISE MASTER REGISTRY & DIGITAL TWIN

## Purpose
Create the authoritative master inventory and digital twin for Litigation 360.

## Generated Structure
$Phase

## Generated Registry Files
- registries\engines-registry.json
- registries\routes-registry.json
- registries\endpoints-registry.json
- registries\workflows-registry.json
- registries\dashboards-registry.json
- registries\documents-registry.json
- registries\integrations-registry.json
- registries\courts-registry.json
- registries\industrialCourt-registry.json
- registries\perkeso-registry.json
- registries\deployment-registry.json
- registries\master-system-registry.json
...
- technical-debt\technical-debt-scan.json

## Generated Docs
- MASTER-SYSTEM-REGISTRY.md
- MASTER-DIGITAL-TWIN.md
- MASTER-DEPENDENCY-MAP.md
- MASTER-IMPACT-ANALYSIS.md
- MASTER-TECHNICAL-DEBT-REPORT.md
- MASTER-INTEGRATION-REGISTRY.md
- MASTER-COURT-REGISTRY.md
- MASTER-PERKESO-REGISTRY.md
"@ | Out-File -LiteralPath (Join-Path $Docs "PHASE10Y0-MASTER-REGISTRY-DIGITAL-TWIN-PROTOCOL.md") -Encoding UTF8
}

$Validate=Join-Path $Validation "validate-phase10Y0.js"
...
$runExit=0
if($Mode -eq "APPLY"){
  Write-Host "Generating Enterprise Master Registry and Digital Twin..."
  node $Script
...
$env:L360_PHASE=$Phase
$env:L360_SCRIPT=$Script
...
if($runExit -eq 0 -and $validateExit -eq 0){
  Write-Host "PHASE 10Y.0 ENTERPRISE MASTER REGISTRY STATUS: PASS" -ForegroundColor Green
}else{
  Write-Host "PHASE 10Y.0 ENTERPRISE MASTER REGISTRY STATUS: FAIL" -ForegroundColor Yellow
}
