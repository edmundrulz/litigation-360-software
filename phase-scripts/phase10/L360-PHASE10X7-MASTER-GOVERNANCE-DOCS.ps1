...
$Gov=Join-Path $Ops "enterprise-governance"
$Docs=Join-Path $Gov "docs"
$Reports=Join-Path $Gov "reports"
$Registries=Join-Path $Gov "registries"
$Checklists=Join-Path $Gov "checklists"
$Logs=Join-Path $Gov "logs"
$Validation=Join-Path $Gov "validation"
$Backups=Join-Path $Gov "backups"
$Script=Join-Path $Root "PHASE10X7-MASTER-GOVERNANCE-DOCS.js"

New-Item -ItemType Directory -Force -Path $Gov,$Docs,$Reports,$Registries,$Checklists,$Logs,$Validation,$Backups | Out-Null
...
Clear-Host
Write-Host "============================================================"
Write-Host "L360 PHASE 10X.7 MASTER GOVERNANCE DOCUMENTATION PACK"
Write-Host "============================================================"
...
- PERKESO Kuala Lumpur — Wisma PERKESO / Jalan Tun Razak
...
$Validate=Join-Path $Validation "validate-phase10X7.js"
...
$runExit=0
if($Mode -eq "APPLY"){
  Write-Host "Generating Master Governance Documentation Pack..."
  node $Script
...
if($runExit -eq 0 -and $validateExit -eq 0){
  Write-Host "PHASE 10X.7 MASTER GOVERNANCE DOCUMENTATION STATUS: PASS" -ForegroundColor Green
}else{
  Write-Host "PHASE 10X.7 MASTER GOVERNANCE DOCUMENTATION STATUS: FAIL" -ForegroundColor Yellow
}
