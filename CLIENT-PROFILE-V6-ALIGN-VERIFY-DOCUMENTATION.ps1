# ============================================================
# LITIGATION 360
# CLIENT PROFILE V6 - ALIGNMENT + VERIFICATION + DOCUMENTATION PATCH
#
# Purpose:
#   Replace frontend\src\pages\Clients.jsx with a cleaner,
#   aligned, professional Malaysian/Singaporean client profile form.
#
# Implements:
#   - Proper 2-column alignment and consistent row flow
#   - Clean labels with extra standalone # symbols removed
#   - IC Colour / Document Class
#   - Blue MyKad auto-confirms Malaysian Citizen
#   - Red auto-confirms Malaysia Permanent Resident
#   - Green auto-confirms Temporary Resident / MyKAS
#   - dd/mm/yyyy DOB display
#   - NRIC DOB extraction, adult/senior categorisation, generation lock
#   - NRIC gender extraction and title/gender validation
#   - Primary/backup phone single-line country-code + number fields
#   - Malaysian phone validation
#   - Availability Until split into date + 24-hour time
#   - Unavailability reason only appears when Availability Until is set
#   - Document Related Reference Notes
#   - Special Remarks / Staff-Lawyer Notes with local audit trail
#   - Verification flags and review status
#   - PowerShell report documentation
#
# Safety:
...
function Write-Step {
    ...
    Write-Host "[CLIENT PROFILE V6] $Message" -ForegroundColor Cyan
}
...
$ClientsBackup = "$ClientsPath.BACKUP_BEFORE_CLIENT_PROFILE_V6_$Stamp"
...
$CssBackup = "$CssPath.BACKUP_BEFORE_CLIENT_PROFILE_V6_$Stamp"
...
function normalizeClient(rawClient) {
  ...
  return {
    ...
    phoneCountryCode: source.phoneCountryCode || "+60 Malaysia",
    phoneNumber: source.phoneNumber || source.phone || "",
    backupPhoneCountryCode: source.backupPhoneCountryCode || "+60 Malaysia",
    ...
    whatsappCountryCode: source.whatsappCountryCode || source.phoneCountryCode || "+60 Malaysia",
...
function buildAuditTrail(form, existingClient, now) {
  ...
  const watchedFields = [
    ...
    ["specialRemarksStaffLawyerNotes", "Special Remarks / Staff-Lawyer Notes"],
...
export default function Clients() {
  ...
  function validateClientForm(payload) {
    ...
    if (payload.titleGenderOverride && !payload.titleOverrideReason.trim()) {
      errors.push("Manual title/gender override requires a reason.");
    }

    if (payload.identityCardColour === "Blue - Malaysian Citizen / MyKad" && payload.residencyStatus !== "Malaysian Citizen") {
      flags.push("Identity Card Colour indicates Malaysian Citizen but documented status differs.");
    }

    if (payload.identityCardColour === "Red - Permanent Resident" && payload.residencyStatus !== "Malaysia Permanent Resident") {
    ...
    if (payload.unavailableUntilDate && !payload.unavailableUntilTime) {
      errors.push("Unavailable Until time is required when Unavailable Until date is set.");
    }

    if (payload.unavailableUntilDate && payload.availabilityReason === "Not Applicable / N/A") {
      errors.push("Reason for Unavailability is required when Unavailable Until is set.");
    }
      ...
      <form className="client-form client-form-v6" onSubmit={saveClient}>
        <div className="form-section">
          ...
          <div className="smart-grid two">
            ...
            <label>
              Title Suffix
              <select value={form.titleSuffix} onChange={(event) => updateForm("titleSuffix", event.target.value)}>
                {TITLE_SUFFIX_OPTIONS.map((option) => (
                  <option key={option} value={option}>{option}</option>
                ))}
              </select>
            </label>
            ...
            <label>
              ...
              <small>NRIC odd final digit = Male. Even final digit = Female.</small>
            </label>
        ...
        <div className="form-section">
          <h3>2. Identification, Date of Birth, Age and Generation</h3>
        ...
        <div className="form-section">
          ...
          <div className="smart-grid two">
            ...
            {unavailabilityIsSet && (
              <label className="full">
                Reason for Unavailability <RequiredMark />
        ...
        <div className="form-section">
          <h3>6. Emergency Contact, Verification and Documentation</h3>
...
Write-Pass "Clients.jsx replaced with Client Profile V6."

$Css = [System.IO.File]::ReadAllText($CssPath)

$MarkerStart = "/* L360 CLIENT PROFILE V6 START */"
$MarkerEnd = "/* L360 CLIENT PROFILE V6 END */"

$CssBlock = @'

/* L360 CLIENT PROFILE V6 START */
...
.client-form-v6 .action-link,
.client-form-v6 .muted-box {
  display: flex !important;
  align-items: center !important;
  min-height: 38px !important;
  padding: 8px 10px !important;
...
.client-form-v6 .important-notes textarea {
  min-height: 110px !important;
  border-color: #d3a12f !important;
  background: #fffaf0 !important;
}

.client-form-v6 .review-flag-box,
.client-form-v6 .audit-box {
...
.client-form-v6 .review-flag-box ul {
  margin: 8px 0 0 !important;
  padding-left: 20px !important;
}

.client-form-v6 .audit-box p {
...
Write-Pass "Client Profile V6 CSS applied."

$ReportFolder = Join-Path $ProjectRoot "_LEOS_CONTROL\reports"
New-Item -ItemType Directory -Path $ReportFolder -Force | Out-Null

$ReportPath = Join-Path $ReportFolder "CLIENT-PROFILE-V6-ALIGNMENT-VERIFICATION-DOCUMENTATION-REPORT-$Stamp.md"
...
Write-Host ""
Write-Pass "CLIENT PROFILE V6 COMPLETE"
Write-Host ""
