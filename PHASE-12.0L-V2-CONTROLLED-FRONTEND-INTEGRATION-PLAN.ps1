...
#
# FIX:
#   Replaces the earlier 12.0L script that failed with:
#   Join-Path : Cannot convert 'System.Object[]' to System.String
#
# PURPOSE:
...
function Write-Step {
    ...
    Write-Host "[PHASE 12.0L V2] $Message" -ForegroundColor Cyan
}
...
$PrototypeFiles = @()
$PrototypeFiles += "LegalManagementShell.jsx"
$PrototypeFiles += "LegalManagementShell.css"
$PrototypeFiles += "firmProfile.config.json"
$PrototypeFiles += "legalNewsLinks.config.json"
$PrototypeFiles += "IMPLEMENTATION-GUIDE.md"

$PrototypeRows = @()
foreach ($File in $PrototypeFiles) {
    $FullPath = Join-Path $PrototypeRoot $File
    $PrototypeRows += [PSCustomObject]@{
        File = $File
        FullPath = $FullPath
...
$PrototypeRows | Export-Csv -Path (Join-Path $IntegrationRoot "PHASE-12.0L-V2-PROTOTYPE-FILE-CHECK.csv") -NoTypeInformation -Encoding UTF8

$PrototypeMissingCount = (($PrototypeRows | Where-Object { $_.Exists -eq $false }) | Measure-Object).Count
$PrototypeReady = $PrototypeMissingCount -eq 0
...
$FrontendAppJsx = Join-Path $FrontendSrc "App.jsx"
$FrontendAppTsx = Join-Path $FrontendSrc "App.tsx"
$FrontendMainJsx = Join-Path $FrontendSrc "main.jsx"
$FrontendMainTsx = Join-Path $FrontendSrc "main.tsx"
$FrontendPages = Join-Path $FrontendSrc "pages"
$FrontendComponents = Join-Path $FrontendSrc "components"

$FrontendCheck = @()
$FrontendCheck += [PSCustomObject]@{ Item="frontend folder"; Path=$FrontendRoot; Exists=(Test-Path -LiteralPath $FrontendRoot -PathType Container) }
$FrontendCheck += [PSCustomObject]@{ Item="frontend/src folder"; Path=$FrontendSrc; Exists=(Test-Path -LiteralPath $FrontendSrc -PathType Container) }
$FrontendCheck += [PSCustomObject]@{ Item="frontend/package.json"; Path=$FrontendPackageJson; Exists=(Test-Path -LiteralPath $FrontendPackageJson -PathType Leaf) }
$FrontendCheck += [PSCustomObject]@{ Item="frontend/src/App.jsx"; Path=$FrontendAppJsx; Exists=(Test-Path -LiteralPath $FrontendAppJsx -PathType Leaf) }
$FrontendCheck += [PSCustomObject]@{ Item="frontend/src/App.tsx"; Path=$FrontendAppTsx; Exists=(Test-Path -LiteralPath $FrontendAppTsx -PathType Leaf) }
$FrontendCheck += [PSCustomObject]@{ Item="frontend/src/main.jsx"; Path=$FrontendMainJsx; Exists=(Test-Path -LiteralPath $FrontendMainJsx -PathType Leaf) }
$FrontendCheck += [PSCustomObject]@{ Item="frontend/src/main.tsx"; Path=$FrontendMainTsx; Exists=(Test-Path -LiteralPath $FrontendMainTsx -PathType Leaf) }
$FrontendCheck += [PSCustomObject]@{ Item="frontend/src/pages folder"; Path=$FrontendPages; Exists=(Test-Path -LiteralPath $FrontendPages -PathType Container) }
$FrontendCheck += [PSCustomObject]@{ Item="frontend/src/components folder"; Path=$FrontendComponents; Exists=(Test-Path -LiteralPath $FrontendComponents -PathType Container) }

$FrontendCheck | Export-Csv -Path (Join-Path $IntegrationRoot "PHASE-12.0L-V2-FRONTEND-STRUCTURE-CHECK.csv") -NoTypeInformation -Encoding UTF8
...
$AppCandidates = @()
$AppCandidates += $FrontendAppJsx
$AppCandidates += $FrontendAppTsx
...
$RouteRows | Export-Csv -Path (Join-Path $IntegrationRoot "PHASE-12.0L-V2-APP-ROUTE-PATTERN-SCAN.csv") -NoTypeInformation -Encoding UTF8
...
if ($PrototypeReady) {
    $CopyFiles = @()
    $CopyFiles += "LegalManagementShell.jsx"
    $CopyFiles += "LegalManagementShell.css"
    $CopyFiles += "firmProfile.config.json"
    $CopyFiles += "legalNewsLinks.config.json"
...
$PlanPath = Join-Path $IntegrationRoot "PHASE-12.0L-V2-CONTROLLED-FRONTEND-INTEGRATION-PLAN.md"
Save-Text -Path $PlanPath -Content $RecommendedMount

# ------------------------------------------------------------
# 7. Create Phase 12.0M draft but locked
# ------------------------------------------------------------
$LockedNextScript = @'
...
#
# This is a draft of the next step after the Phase 12.0L V2 plan is reviewed.
# Phase 12.0M would:
# 1. Backup App.jsx
...
# ------------------------------------------------------------
$Report = @"
# PHASE 12.0L V2 CONTROLLED FRONTEND INTEGRATION PLAN REPORT
...
notepad "_LEOS_CONTROL\feature-exploration\ui-integration-plan\PHASE-12.0L-V2-CONTROLLED-FRONTEND-INTEGRATION-PLAN.md"

Do not run active integration yet.

## Files Created

- _LEOS_CONTROL\feature-exploration\ui-integration-plan\PHASE-12.0L-V2-PROTOTYPE-FILE-CHECK.csv
- _LEOS_CONTROL\feature-exploration\ui-integration-plan\PHASE-12.0L-V2-FRONTEND-STRUCTURE-CHECK.csv
- _LEOS_CONTROL\feature-exploration\ui-integration-plan\PHASE-12.0L-V2-APP-ROUTE-PATTERN-SCAN.csv
- _LEOS_CONTROL\feature-exploration\ui-integration-plan\PHASE-12.0L-V2-CONTROLLED-FRONTEND-INTEGRATION-PLAN.md
- _LEOS_CONTROL\feature-exploration\ui-integration-plan\candidate-frontend-files
...
$ReportPath = Join-Path $ReportRoot "PHASE-12.0L-V2-CONTROLLED-FRONTEND-INTEGRATION-PLAN-REPORT.md"
Save-Text -Path $ReportPath -Content $Report

Write-Host ""
Write-Pass "PHASE 12.0L V2 CONTROLLED FRONTEND INTEGRATION PLAN COMPLETE"
Write-Host ""
Write-Host "Open report:" -ForegroundColor Cyan
Write-Host "notepad `"_LEOS_CONTROL\reports\PHASE-12.0L-V2-CONTROLLED-FRONTEND-INTEGRATION-PLAN-REPORT.md`""
Write-Host ""
Write-Host "Open integration plan:" -ForegroundColor Cyan
Write-Host "notepad `"_LEOS_CONTROL\feature-exploration\ui-integration-plan\PHASE-12.0L-V2-CONTROLLED-FRONTEND-INTEGRATION-PLAN.md`""
Write-Host ""
...
Write-Host ""
Write-Pass "Paste the Phase 12.0L V2 report back into ChatGPT."
