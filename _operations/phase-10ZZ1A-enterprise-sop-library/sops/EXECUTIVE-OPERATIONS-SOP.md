...
## Purpose
Control executive dashboard, reporting, decision escalation and management visibility.
...
```cmd
C:\Users\jep_edmundrulz\litigation-360-workspace\litigation-360-software\_operations\phase-10ZZ1A-enterprise-sop-library\sops\EXECUTIVE-OPERATIONS-SOP.md
```
...
```text
Apply the Executive Operations SOP. Confirm project root, create backups where needed, generate evidence, run validation, produce PASS/FAIL report, and do not perform destructive actions without approval.
```

```text
Review the Executive Operations SOP failure. Identify failed step, affected file path, missing evidence, validation status, recovery action, escalation owner and next command to run.
```
