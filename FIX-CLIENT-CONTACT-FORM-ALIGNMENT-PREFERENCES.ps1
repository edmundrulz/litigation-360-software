# ============================================================
# LITIGATION 360
# CLIENT CONTACT FORM PROFESSIONAL ALIGNMENT + CONTACT PREFERENCE PATCH
#
# Purpose:
#   Replaces frontend\src\pages\Clients.jsx with an enhanced,
#   aligned client registration/contact form:
#   - Header "Client's Details"
#   - Professional balanced form grid
#   - Malaysia +60 first/default, other country codes searchable/alphabetical
#   - Malaysian phone validation hint and warning
#   - One WhatsApp number by default; second WhatsApp field only when checked
#   - Contact preference 1st / 2nd / 3rd choice
#   - Preferred contact hours
#   - Away / unreachable duration
#   - Next of kin / emergency contact
#   - Required/missing field indicators
#   - N/A / Unknown / To be confirmed handling
#
# Safety:
...
function Write-Step {
    ...
    Write-Host "[CLIENT CONTACT FORM PATCH] $Message" -ForegroundColor Cyan
}
...
$ClientsBackup = "$ClientsPath.BACKUP_BEFORE_CLIENT_CONTACT_FORM_PATCH_$Stamp"
...
$CssBackup = "$CssPath.BACKUP_BEFORE_CLIENT_CONTACT_FORM_PATCH_$Stamp"
...
const EMPTY_CLIENT = {
  ...
  phoneNumber: "",
  phoneInfoStatus: "Available",

  whatsappSameAsPhone: true,
  whatsappCountryCode: "+60 Malaysia",
  whatsappNumber: "",
  whatsappInfoStatus: "Available",
...
function readLocalClients() {
  ...
  LOCAL_STORAGE_KEYS.forEach((key) => {
    try {
      ...
      // ignore invalid old local storage
    }
  });
...
function normalizeClient(rawClient) {
  ...
  return {
    ...
    phoneCountryCode,
    phoneNumber,
    phoneInfoStatus: source.phoneInfoStatus || "Available",

    whatsappSameAsPhone: source.whatsappSameAsPhone !== undefined ? Boolean(source.whatsappSameAsPhone) : true,
    whatsappCountryCode,
    whatsappNumber,
    whatsappInfoStatus: source.whatsappInfoStatus || "Available",
    ...
    whatsappNotes: source.whatsappNotes || "",
    ...
    preferredContactTimeNote: source.preferredContactTimeNote || "",
    unreachableFrom: source.unreachableFrom || "",
    unreachableTo: source.unreachableTo || "",
    unreachableReason: source.unreachableReason || "",
    ...
    addressInfoStatus: source.addressInfoStatus || "Available",
...
export default function Clients() {
  ...
  function validateClientForm(payload) {
    ...
    if (payload.phoneInfoStatus === "Available" && !payload.phoneNumber.trim()) {
      errors.push("Phone Number is marked Available but is empty.");
    }

    if (payload.whatsappInfoStatus === "Available" && !payload.whatsappNumber.trim()) {
      errors.push("WhatsApp Number is marked Available but is empty.");
    }

    if (isMalaysiaCountryCode(payload.phoneCountryCode) && payload.phoneNumber && !isValidMalaysiaMobile(payload.phoneNumber)) {
      errors.push("Malaysian Phone Number should start with 01 and use digits only, example 0123456789.");
    }

    if (isMalaysiaCountryCode(payload.whatsappCountryCode) && payload.whatsappNumber && !isValidMalaysiaMobile(payload.whatsappNumber)) {
      errors.push("Malaysian WhatsApp Number should start with 01 and use digits only, example 0123456789.");
    }
    ...
    if (payload.unreachableFrom && payload.unreachableTo && payload.unreachableFrom > payload.unreachableTo) {
      errors.push("Unreachable From date/time cannot be after Unreachable Until date/time.");
    }
  ...
  async function saveClient(event) {
    ...
    if (formErrors.length > 0) {
      showStatus("Please fix the highlighted missing or invalid client details before saving.", "error");
      return;
    }
    ...
    try {
      ...
      showStatus(
        editingId
          ? "Client contact details successfully modified and saved."
          : "Client contact details successfully entered, received and saved.",
        "success"
      );
    } catch (error) {
  ...
  const filteredClients = useMemo(() => {
    ...
    return clients.filter((client) => {
      ...
      const searchable = [
        ...
        normalized.whatsapp2CountryCode,
        normalized.whatsapp2Number,
    ...
    <section className="client-module">
      <div className="client-module-header">
        <div>
          ...
          <p>
            Searchable by initials, gender, name, surname, NRIC/passport,
            ethnicity, nationality, phone, WhatsApp, email, contact preference and address.
          </p>
        </div>
      ...
      <form className="client-form" onSubmit={saveClient}>
        ...
        <h3>Phone, WhatsApp and Contact Preference</h3>

        <div className="client-form-grid">
          <label>
            Phone Info Status
            <select
              value={form.phoneInfoStatus}
              onChange={(event) => updateForm("phoneInfoStatus", event.target.value)}
            >
              {INFO_STATUS_OPTIONS.map((option) => (
          ...
          <label>
            ...
            />
            <small>Malaysia +60 is the default. Search by country or code.</small>
          </label>

          <label>
            Phone Number <NumberMark />
            <input
          ...
          <label>
            ...
            <select
              value={form.whatsappInfoStatus}
              onChange={(event) => updateForm("whatsappInfoStatus", event.target.value)}
            >
              {INFO_STATUS_OPTIONS.map((option) => (
                <option key={option} value={option}>{option}</option>
              ))}
          ...
          <label>
            ...
            {malaysiaWhatsappWarning && (
              <small className="field-warning">Check format: Malaysian WhatsApp mobile should start with 01.</small>
            )}
          </label>
          ...
          <label className="client-checkbox-label">
            ...
            />
            Client has more than one mobile / WhatsApp number
          </label>

          {form.hasSecondWhatsapp && (
            <>
              <label>
                Second WhatsApp Country Code
                <input
                  list="client-country-code-options"
          ...
          <label className="client-span-2">
            WhatsApp Notes / Availability
            <textarea
              value={form.whatsappNotes}
              onChange={(event) => updateForm("whatsappNotes", event.target.value)}
              placeholder="Example: only this number has WhatsApp; reachable after 6pm; abroad number active only on weekends."
            />
          </label>
          ...
          <label className="client-span-2">
            ...
            <textarea
              value={form.preferredContactTimeNote}
              onChange={(event) => updateForm("preferredContactTimeNote", event.target.value)}
              placeholder="Example: prefers AM only, unavailable during court hours, WhatsApp after office hours."
            />
          </label>
          ...
          <label>
            ...
            <input
              type="datetime-local"
              value={form.unreachableFrom}
              onChange={(event) => updateForm("unreachableFrom", event.target.value)}
            />
          </label>
          ...
          <label>
            ...
            <input
              type="datetime-local"
              value={form.unreachableTo}
              onChange={(event) => updateForm("unreachableTo", event.target.value)}
            />
          </label>

          <label className="client-span-2">
            Away / Unreachable Reason or Notice
            <textarea
              value={form.unreachableReason}
              onChange={(event) => updateForm("unreachableReason", event.target.value)}
              placeholder="Example: overseas from date to date; use emergency contact during this period."
            />
          </label>
        </div>
        ...
        <div className="client-form-grid">
          <label>
            Address Info Status
            <select
              value={form.addressInfoStatus}
              onChange={(event) => updateForm("addressInfoStatus", event.target.value)}
            >
              {INFO_STATUS_OPTIONS.map((option) => (
                <option key={option} value={option}>{option}</option>
              ))}
      ...
      <div className="client-search-row">
        <label>
          ...
          <input
            ...
            placeholder="Search initials, gender, name, surname, NRIC/passport, ethnicity, nationality, phone, WhatsApp, email or emergency contact"
          />
        </label>
      </div>
        ...
        <table className="client-table">
          ...
          <tbody>
            ...
            {filteredClients.map((client) => {
              ...
              const secondWhatsappForLinks = normalizePhoneForLinks(normalized.whatsapp2CountryCode, normalized.whatsapp2Number);
              ...
              const whatsapp2Link = secondWhatsappForLinks ? "https://wa.me/" + secondWhatsappForLinks : "";
              ...
              const ethnicityDisplay =
                ...
                normalized.ethnicity === "Other Singapore Ethnicity"
                  ...
                  : normalized.ethnicity;
              const unreachablePeriod =
                normalized.unreachableFrom || normalized.unreachableTo
                  ? [normalized.unreachableFrom || "From not set", normalized.unreachableTo || "Until not set"].join(" to ")
                  : "-";
                  ...
                  <td>
                    {whatsappForLinks ? (
                      <div className="client-contact-actions">
                        ...
                        {normalized.hasSecondWhatsapp && whatsapp2Link && (
                          <a href={whatsapp2Link} target="_blank" rel="noreferrer">
                            WhatsApp 2
                          </a>
                        )}
                      </div>
                    ) : (
                      normalized.whatsappInfoStatus
                    )}
                  </td>
                  <td>
                    {normalized.preferredContact1 || "-"}
                    ...
                    <br />
                    3rd: {normalized.preferredContact3 || "-"}
                  </td>
                  <td>
      ...
      <p className="client-footnote">
        * indicates a required field. # marks number-based fields. NRIC / IC numbers are masked in the table for privacy.
        Passport numbers are partially masked. Gender is auto-suggested from the final NRIC digit and can still be manually adjusted.
        Document file names are matched to the client record; secure scanned document storage requires backend upload support.
      </p>
    </section>
  );
}
...
Write-Pass "Clients.jsx replaced with aligned contact form, validation, contact preferences, away notice and emergency contact."

$Css = [System.IO.File]::ReadAllText($CssPath)

$MarkerStart = "/* L360 CLIENT CONTACT FORM V3 START */"
$MarkerEnd = "/* L360 CLIENT CONTACT FORM V3 END */"

$CssBlock = @'

/* L360 CLIENT CONTACT FORM V3 START */
...
/* L360 CLIENT CONTACT FORM V3 END */
'@

if ($Css.Contains($MarkerStart)) {
    Write-Warn "Existing client contact form V3 CSS found. Replacing old block."
...
Write-Pass "Client contact form CSS applied."

$ReportFolder = Join-Path $ProjectRoot "_LEOS_CONTROL\reports"
New-Item -ItemType Directory -Path $ReportFolder -Force | Out-Null

$ReportPath = Join-Path $ReportFolder "CLIENT-CONTACT-FORM-PATCH-REPORT-$Stamp.md"
...
Write-Host ""
Write-Pass "CLIENT CONTACT FORM PATCH COMPLETE"
Write-Host ""
